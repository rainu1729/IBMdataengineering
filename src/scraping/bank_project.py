...
from src.config.config import BANK_URL, BANK_EXCHANGE_RATE_URL, BANK_DB, BANK_TABLE, BANK_SCHEMA, BANK_OUTPUT
...
def transform(df,csv_path):
    ...
    df["MC_GBP_Billion"] = df["MC_USD_Billion"].astype(float) * exchange_rate_df[exchange_rate_df['Currency'] == 'GBP']['Rate'].values[0]
    df["MC_EUR_Billion"] = df["MC_USD_Billion"].astype(float) * exchange_rate_df[exchange_rate_df['Currency'] == 'EUR']['Rate'].values[0]
    df["MC_INR_Billion"] = df["MC_USD_Billion"].astype(float) * exchange_rate_df[exchange_rate_df['Currency'] == 'INR']['Rate'].values[0]
    return df
